...
public class Parser {
    ...
    try (BufferedReader reader = new BufferedReader(new FileReader(filename))) {
        ...
        while ((line = reader.readLine()) != null) {
            // Verifica se a linha é vazia ou inválida
            if (line.trim().isEmpty()) {
                    continue;  // Ignorar linhas vazias ou só com espaços
            }
            try {
                parseLine(line);  // Processa a linha (método fictício, substitua pela lógica real)
            } catch (UnrecognizedEntryException e) {
                throw new UnrecognizedEntryException("Entrada não reconhecida no ficheiro: " + line);
            }
    ...
    private boolean linhaVaziaOuInvalida(String line) {
        // Verifica se a linha é nula ou está vazia
